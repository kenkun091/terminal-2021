...
class AlgoStrategy(gamelib.AlgoCore):
    ...
    def new_strategy(self, game_state):
        ...
        if game_state.get_resource(MP) >= 16 + game_state.turn_number * 0.1:
        ...
        if suicide_direction is not None and self.attack_next_round == False:
            ...
            # check if hole 2nd spot over from corner position
                if game_state.contains_stationary_unit([1, 14]):
                    # 2 wave scout rush
                    game_state.attempt_spawn(SCOUT, [16, 2], int(game_state.get_resource(MP) / 2))  # half in first wave
            ...
            # check if hole 2nd spot over from corner position
                if game_state.contains_stationary_unit([26, 14]):
                    # 2 wave scout rush
                    game_state.attempt_spawn(SCOUT, [11, 2], int(game_state.get_resource(MP) / 2))  # half in first wave
    ...
    def defense_strategy(self, game_state, suicide_direction=None):
        ...
        turn_1_walls = [[1, 13], [2, 13], [25, 13], [26, 13], [27, 13], [3, 12], [4, 11], [5, 10], [6, 9], [21, 9], [7, 8], [20, 8], [8, 7], [19, 7], [9, 6], [18, 6], [10, 5], [17, 5], [11, 4], [16, 4], [12, 3], [13, 3], [14, 3], [15, 3]]
        ...
        support_points = [[19, 8], [18, 7], [17, 6], [16, 5], [15, 4], [13, 2], [14, 2]]
        ...
        for location in reversed([x for x in game_state.game_map]):
            if game_state.get_resource(SP) < 2:    # skip door upgrades
                break
            for unit in game_state.game_map[location]:
    ...
    def stall_with_interceptors(self, game_state):
        ...
        while game_state.get_resource(MP) >= game_state.type_cost(INTERCEPTOR)[MP] + 2 and len(deploy_locations) > 0:
